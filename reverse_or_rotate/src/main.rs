...
//    while let Some(chunk) = chunks.next() {
//        if chunk.iter().fold(0, |acc, x| acc + (x.to_digit(10).unwrap() as f64).powi(3) as i32) % 2 == 0 {
//            result += &chunk.iter().rev().fold(String::new(), |acc, x| acc + &x.to_string());
//        } else {
//            result += {
//                let (head, tail) = chunk.split_at(1);
//
//                &(tail.iter().fold(String::new(), |acc, x| acc + &x.to_string()) + &head.iter().fold(String::new(), |acc, x| acc + &x.to_string()))
//
//            }
//        }
//    }
